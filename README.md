...
- `authorized_keys` (*optional*): provide client's public SSH key to secure `root` ssh login. Also possible with [`ssh_key=`](https://wiki.alpinelinux.org/wiki/Diskless_Mode#Documentation_on_kernel_options) cmdline parameter.
...
[^2]: Temporarily remove `root=*` statement from cmdline parameters list to disable disk-based boot mode.
